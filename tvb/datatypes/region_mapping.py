...
class RegionMapping(object):
    ...
    def _find_summary_info(self):
        ...
        summary.update({"Source Surface": self.surface.display_name,
                        ...
                        "Connectivity GID": self.gid,
                        "Connectivity": self.display_name})
        return summary
...
class RegionVolumeMapping(RegionMapping):
    ...
    def write_data_slice(self, data):
        ...
        if self.apply_corrections:
            ...
            data[data >= self.number_of_regions] = -1
            data[data < -1] = -1
    ...
    def get_voxel_region(self, x_plane, y_plane, z_plane):
        ...
        if voxel != -1:
            return self.region_labels[int(voxel)]
        else:
            return 'background'
    ...
    def get_mapped_array_volume_view(self, mapped_array, x_plane, y_plane, z_plane, mapped_array_slice=None, **kwargs):
        ...
        if mapped_array_slice:
            ...
            measure = mapped_array.get_data('mapping', matrix_slice)
        else:
            measure = mapped_array.get_data('mapping')

        if measure.shape != (self.number_of_regions, ):
